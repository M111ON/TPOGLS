...
#define GEO_CACHE_SLOTS   4096  /* S4: 256→4096, covers 4096-key bench without collision storm */
...
static inline GeoCacheSlot *geocache_get(GeoCache *c, uint64_t key) {
    /* S4: skip bit0 (bench keys are always-odd → bit0=1 wastes half slots) */
...
static inline void geocache_invalidate(GeoCache *c) {
    /* S4: soft-invalidate — clear valid flag only, preserve slot layout.
     * memset(0) was nuking L1 on every rewind → 0% hit rate.
     * KV holds truth; L1 valid=0 forces re-populate on next read miss. */
    for (int _i = 0; _i < GEO_CACHE_SLOTS; _i++)
        c->slots[_i].valid = 0;
    c->hits = c->misses = 0;
}
...
/* ── Rewind variants ────────────────────────────────────────────── */
/* Hard rewind: flush L1 (original behaviour — commit boundary) */
static inline void pogls_rewind(PoglsCtx *ctx) {
    geocache_invalidate(&ctx->l1);
}
/* Soft rewind: keep L1 warm — use when KV truth is trusted
 * and you want read performance to survive the rewind.
 * S4: bench read phase benefits immediately. */
static inline void pogls_rewind_soft(PoglsCtx *ctx) {
    /* Reset counters only — L1 data stays warm, KV truth unchanged.
     * Bench snapshots hits/misses after this call to isolate read phase. */
    ctx->l1.hits   = 0;
    ctx->l1.misses = 0;
}
