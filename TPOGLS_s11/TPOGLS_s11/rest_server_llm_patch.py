...
try:
    from geo_net_py import route_layer
    _HAS_GEONET = True
except ImportError:
    _HAS_GEONET = False
...
class WalletLoadBody(BaseModel):
    wallet_b64: str        # base64-encoded .pogwallet blob
    src_b64:    str        # base64-encoded raw source bytes (file_idx=0)
    ctx_id:     str = "default"

class WalletLayerBody(BaseModel):
    wallet_b64: str
    src_b64:    str
    layer:      str        # layer name e.g. "layer_0"
    ctx_id:     str = "default"
...
# ═══════════════════════════════════════════════════════════════
# ROUTES — S23 wallet streaming (WeightStream)
# ═══════════════════════════════════════════════════════════════

def _wallet_not_available():
    raise HTTPException(503, "wallet stack not installed (pogls_mount / pogls_weight_stream)")

def _parse_wallet(wallet_b64: str, src_b64: str):
    """Decode b64 inputs → VirtualFile. Raises HTTPException on bad input."""
    if not _HAS_WALLET:
...
def wallet_load(body: WalletLoadBody):
    """
    Mount .pogwallet blob → return layer names, shapes, dtypes.

    Request:  { "wallet_b64": "...", "src_b64": "...", "ctx_id": "..." }
    Response: { "layers": {"layer_0": {"shape": [N], "dtype": "float32", "bytes": M}}, ... }
    """
    import numpy as np
    ...
    try:
        ...
        for i, name in enumerate(loader.keys()):
            ...
            if _HAS_GEONET:
                info["routing"] = route_layer(i, arr.nbytes)
            layers[name] = info
    except Exception as e:
...
def wallet_layer(body: WalletLayerBody):
    """
    Stream single layer from .pogwallet → return tensor bytes (base64).

    Request:  { "wallet_b64": "...", "src_b64": "...", "layer": "layer_0" }
    Response: { "data_b64": "...", "shape": [...], "dtype": "float32", "bytes": N }
    """
    import numpy as np
    vf = _parse_wallet(body.wallet_b64, body.src_b64)
    ...
    log.info("wallet/layer ctx=%s layer=%s shape=%s ms=%.1f",
             body.ctx_id, body.layer, arr.shape, elapsed_ms)
    return JSONResponse({
