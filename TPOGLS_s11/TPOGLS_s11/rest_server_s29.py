"""
rest_server_s29.py — POGLS REST API (S29)
══════════════════════════════════════════
Builds on S28. Core change: annotate=true → NDJSON stream (StreamingResponse)
instead of building chunk_map in memory.

S29 upgrades:
  • POST /wallet/layer + annotate=true
      → StreamingResponse (NDJSON) — memory O(1) regardless of layer size
      → header: X-Chunk-Count, X-Layer-State, X-Geo-Backend
      → each line: {"chunk_i":…,"addr":…,"spoke":…,"inv_spoke":…,
                    "mirror_mask":…,"is_audit":…}
      → last line: {"_summary": true, "n_chunks":…, "spoke_dist":[…],
                    "qrpn_state_after":…, "verify_pass":…}
  • POST /wallet/layer + annotate=false  — unchanged (JSON, fast)
  • POST /wallet/load                    — unchanged (S28 route_layer_fast)
  • GET  /wallet/layer/stream            — GET variant for curl/EventSource friendly
  • /health reports iter_streaming: true/false

Memory model:
  S28 annotate=true:  RAM ≈ n_chunks * ~200B  (full chunk_map in list)
  S29 annotate=true:  RAM ≈ BATCH * ~200B     (512 records = ~100KB ceiling)
"""
...
# ── geo backend ────────────────────────────────────────────────────
try:
    from geo_net_ctypes_s29 import GeoNetCtxFast as GeoNetCtx, _LIB as _GEONET_LIB
    _HAS_GEONET  = True
    ...
    try:
        from geo_net_ctypes import GeoNetCtxFast as GeoNetCtx  # type: ignore[no-redef]
        from geo_net_ctypes import _LIB as _GEONET_LIB          # type: ignore[assignment]
        _HAS_GEONET  = True
...
_NDJSON_BATCH = int(os.environ.get("POGLS_NDJSON_BATCH", 512))  # tune RAM/latency tradeoff
...
def _ndjson_stream(
    ...
    """
    Generator → StreamingResponse body.

    Yields:
      • one NDJSON line per chunk  ({"chunk_i":…, "addr":…, …}\\n)
      • final summary line         ({"_summary":true, …}\\n)

    memory ceiling = BATCH * ~200B (configurable via POGLS_NDJSON_BATCH)
    """
    n_chunks   = 0
    spoke_dist = [0] * 6
    audit_cnt  = 0

    for rec in geo_ctx.iter_chunks_stream(file_idx, n_bytes):
...
def _ndjson_stream_gzip(geo_ctx, file_idx, n_bytes, layer) -> Iterator[bytes]:
    """gzip-compressed NDJSON stream — same O(1) memory."""
    buf  = io.BytesIO()
    gz   = gzip.GzipFile(fileobj=buf, mode="wb")
    for chunk in _ndjson_stream(geo_ctx, file_idx, n_bytes, layer):
        gz.write(chunk)
        gz.flush()
        buf.seek(0)
        data = buf.read()
...
async def lifespan(app: FastAPI):
    log.info("POGLS REST server (S29) starting — geo_backend=%s iter=%s port=%d",
...
app = FastAPI(
    title="POGLS v4 REST API (S29)",
    version="1.29.0",
    description="NDJSON streaming for annotate=true — O(1) memory per layer",
    lifespan=lifespan,
)
...
class WalletLayerBody(BaseModel):
    ...
    annotate:  bool = False
    compress:  bool = False   # S29: gzip NDJSON if True + annotate
    ctx_id:    str  = "default"


# ── POST /wallet/load (S28 unchanged) ─────────────────────────────
...
def wallet_load(body: WalletLoadBody):
    ...
    log.info("wallet/load ctx=%s layers=%d ms=%.1f", body.ctx_id, len(layers), elapsed_ms)
...
# ── POST /wallet/layer — S29 core ─────────────────────────────────
#
# annotate=false  → JSON response (same as S28, fast)
# annotate=true   → StreamingResponse NDJSON (NEW — O(1) memory)
...
def wallet_layer(body: WalletLayerBody):
    ...
    # ── annotate=true: stream NDJSON ──────────────────────────────
    if body.annotate and _HAS_GEONET and rec.geo_ctx is not None:
        log.info("wallet/layer STREAM ctx=%s layer=%s compress=%s backend=%s",
                 body.ctx_id, body.layer, body.compress, _GEO_BACKEND)
    ...
    # ── annotate=false: JSON (fast path, S28-compatible) ──────────
    routing   = None
    chunk_map = None
    if _HAS_GEONET and rec.geo_ctx is not None:
        summary, cm = rec.geo_ctx.route_layer_annotated_fast(file_idx, arr.nbytes)
        routing = summary
        # annotate=false → don't include chunk_map in JSON response
        # (caller uses streaming endpoint if they need per-chunk data)

    elapsed_ms = round((time.monotonic() - t0)*1000, 2)
    log.info("wallet/layer JSON ctx=%s layer=%s qrpn=%s backend=%s ms=%.1f",
             body.ctx_id, body.layer,
...
# ── GET /wallet/layer/stream — curl/EventSource friendly ──────────
# Query params mirror WalletLayerBody (wallet + src via query is unwieldy
# for large payloads — this endpoint is mainly for quick streaming tests).

@app.get("/wallet/layer/stream")
def wallet_layer_stream(
    ctx_id:   str  = Query("default"),
    layer:    str  = Query(...),
    compress: bool = Query(False),
):
    """
    Thin GET wrapper — requires wallet already loaded in ctx (future S30 mount).
    Currently returns 501 until persistent wallet mount lands.
    """
    raise HTTPException(501,
        "Persistent wallet mount not yet implemented. "
        "Use POST /wallet/layer with annotate=true instead.")


# ── unchanged endpoints (S23/S27/S28) ─────────────────────────────
...
def health():
    return {
        ...
        "contexts": len(_contexts), "version": "S29",
        ...
        "endpoints": [
            ...
            "POST /wallet/layer  [S29: annotate=false → JSON | annotate=true → NDJSON stream]",
            "POST /wallet/layer  [S29: compress=true → gzip NDJSON]",
            "GET  /wallet/layer/stream  [S30: persistent mount — pending]",
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("rest_server_s29:app", host=_HOST, port=_PORT,
