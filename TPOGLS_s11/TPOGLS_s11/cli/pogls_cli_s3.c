 ...
 * Stream decoded bytes -> stdout. Zero intermediate buffer.
 * cb_stdout fires per 64B Diamond Block via pogls_stream_run().
 */
static void cb_stdout(const uint8_t *chunk, uint32_t len, void *user)
{
    (void)user;
...
{
    if (argc < 3) {
        fprintf(stderr, "usage: pogls cat <input.pogls> [--chunk-start N] [--chunk-end N]\n");
        return 1;
    }
    PoglsFile pf;
    if (pogls_open(&pf, argv[2]) != 0) return 1;
    ...
    uint32_t total       = pf.fi.hdr->chunk_count;
    const char *cs_s     = arg_get(argc, argv, "--chunk-start");
    const char *ce_s     = arg_get(argc, argv, "--chunk-end");
    uint32_t chunk_start = cs_s ? (uint32_t)strtoul(cs_s, NULL, 0) : 0;
    uint32_t chunk_end   = ce_s ? (uint32_t)strtoul(ce_s, NULL, 0) : (total > 0 ? total - 1 : 0);

    if (chunk_start >= total || chunk_end >= total || chunk_start > chunk_end) {
        fprintf(stderr, "error: chunk range [%u..%u] invalid (total=%u)\n",
                chunk_start, chunk_end, total);
        pogls_close(&pf); return 1;
    }

    PoglsStream s;
    int r = pogls_stream_init(&s, &pf.rc, chunk_start, chunk_end);
    if (r != STREAM_OK) {
        fprintf(stderr, "error: stream init failed (%d)\n", r);
        pogls_close(&pf); return 1;
    }

    r = pogls_stream_run(&s, cb_stdout, NULL);
    fflush(stdout);

    uint32_t emitted = pogls_stream_progress(&s);
    fprintf(stderr, "cat: chunks %u..%u -> %u streamed  |  %s\n",
            chunk_start, chunk_end, emitted,
            r == STREAM_DONE ? "OK" : "ERR");

    pogls_close(&pf);
    return (r == STREAM_DONE) ? 0 : 1;
}

/* pogls info <input.pogls>
 * Parse 32B header only — O(1), no decompress, no alloc.
 * X-ray view: magic, version, sizes, comp_ratio, index_overhead.
 */
static int cmd_info(int argc, char **argv)
{
    if (argc < 3) {
        fprintf(stderr, "usage: pogls info <input.pogls>\n");
        return 1;
    }
    ...
    PoglsBlobHeader bh;
    size_t got = fread(&bh, 1, sizeof(bh), f);
    fseek(f, 0, SEEK_END);
    long blob_total = ftell(f);
    fclose(f);

    if (got < sizeof(bh)) { fprintf(stderr, "error: file too small\n"); return 1; }

    double comp_ratio = bh.orig_size > 0
        ? 100.0 * ((long long)bh.orig_size - bh.comp_bytes) / (long long)bh.orig_size : 0.0;
    double blob_ratio = bh.orig_size > 0 ? 100.0 * blob_total / (long long)bh.orig_size : 0.0;
    double overhead   = blob_total  > 0  ? 100.0 * bh.fidx_bytes / blob_total : 0.0;

    printf("=== POGLS INFO: %s ===\n", argv[2]);
    printf("  magic       : 0x%08X  %s\n", bh.magic,
           bh.magic == POGLS_BLOB_MAGIC ? "PGLS OK" : "INVALID");
    printf("  version     : %u  %s\n", bh.version,
           bh.version == POGLS_BLOB_VERSION ? "OK" : "MISMATCH");
    printf("  layout_sig  : 0x%08X\n", bh.layout_sig);
    printf("\n");
    printf("  orig_size   : %llu B  (%.2f KB)\n",
           (unsigned long long)bh.orig_size, bh.orig_size / 1024.0);
    printf("  chunk_count : %u  (64B DiamondBlocks)\n", bh.chunk_count);
    printf("  fidx_bytes  : %u  (index)\n",  bh.fidx_bytes);
    printf("  comp_bytes  : %u  (stream)\n", bh.comp_bytes);
    printf("  blob_total  : %ld B\n", blob_total);
    printf("\n");
    printf("  comp_savings: %+.1f%%  (orig vs comp_bytes)\n", comp_ratio);
    printf("  blob/orig   : %.1f%%   (total blob size vs original)\n", blob_ratio);
    printf("  index_share : %.1f%%   (fidx overhead in blob)\n", overhead);

    if (bh.magic != POGLS_BLOB_MAGIC || bh.version != POGLS_BLOB_VERSION) {
        fprintf(stderr, "\nwarning: header validation failed\n");
        return 1;
    }
    return 0;
}
...
static void usage(void)
{
    fprintf(stderr,
        ...
        "\n"
        "  pogls encode  <input>        <output.pogls>\n"
        "  pogls decode  <input.pogls>  <o>\n"
        "  pogls verify  <input.pogls>\n"
        "  pogls read    <input.pogls>  --offset <bytes> --len <bytes>\n"
        "  pogls cat     <input.pogls>  [--chunk-start N] [--chunk-end N]\n"
        "\n"
        "  read: O(1) seek -- decodes only requested range.\n"
        "  cat:  zero-buffer stream -> stdout, 64B per chunk.\n"
    );
}
