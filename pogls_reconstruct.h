                                           ...
                                           uint32_t           *bytes_out)
{
    ...
    /* ── 4. QRPN guard on checksum (seq as addr) ────────────────── */
    int qfail = 0;
    /* phi_scatter_hex from pogls_qrpn_phaseE: verify stored guard */
    uint32_t expected_guard = phi_scatter_hex(
        (uint64_t)chk ^ (uint64_t)rec->seq);
    if (expected_guard != rec->rec_crc) {
        /* rec_crc doubles as QRPN guard on (checksum ^ seq) */
