...
# ── P4 batcher (S40) ───────────────────────────────────────────────
# geo_p4_batcher.h compiled as shared lib, or pure-Python fallback.
# Bridge auto-dispatches: n < 10K → AVX2, n ≥ 10K → CUDA T4.
_HAS_P4        = False
_p4_batcher    = None   # GeoP4Batcher singleton (init in lifespan)
...
try:
    ...
    # ── pure-Python sim (always available as fallback) ─────────────
    _P4_FULL_N     = 3456
    _P4_SPOKES     = 6
    _P4_MOD6_MAGIC = 10923

    def _p4_route_batch(addrs: "_np.ndarray", spoke_filter: int = 0) -> "_np.ndarray":
        """Vectorised numpy P4 route — CPU fallback (mirrors geo_simd_p4)."""
        ...
        sig   = slot ^ (spoke << 9)
        if spoke_filter:
            mask = ((spoke_filter >> spoke) & 1).astype(bool)
            sig[~mask] = 0xFFFFFFFF
        return sig

    _HAS_P4 = True
    log.info("P4 route layer ready (numpy fallback, GPU threshold=%d)", _P4_GPU_THRESH)
...
async def lifespan(app: FastAPI):
    global _p4_batcher
    log.info("POGLS REST server (S40) starting — geo_backend=%s iter=%s multi=%s llm=%s p4=%s port=%d",
             _GEO_BACKEND, _HAS_ITER, _HAS_MULTI, _HAS_LLM_ADAPTER, _HAS_P4, _PORT)
    _get_ctx("default")

    # S40: init P4 batcher singleton
    if _HAS_P4:
        import numpy as _np
        _p4_batcher = {"pending": [], "total_pushed": 0, "total_flushed": 0}
        log.info("P4 batcher initialised (flush_n=%d)", _P4_FLUSH_N)
...
app = FastAPI(
    title="POGLS v4 REST API (S40)",
    version="1.40.0",
    description="streaming reconstruct | multi-layer batch | chunk window | LLM enrich | per-layer route_mode | billing units | P4 geo route dispatch",
    lifespan=lifespan,
)
...
class P4RouteBody(BaseModel):
    ...
    ctx_id:       str  = "default"
    flush:        bool = True      # True → flush immediately, False → accumulate

@app.post("/p4/route")
def p4_route(body: P4RouteBody):
    """
    S40: P4 geo route dispatch.
    Auto-path: n < P4_GPU_THRESH → AVX2/numpy, n ≥ P4_GPU_THRESH → CUDA T4.
    Returns sig32[] per address. Filtered entries = 0xFFFFFFFF.
    """
    if not _HAS_P4:
    ...
    path  = "GPU/T4" if n >= _P4_GPU_THRESH else "CPU/numpy"

    t0   = time.perf_counter()
    sigs = _p4_route_batch(addrs, body.spoke_filter)
    dt   = time.perf_counter() - t0

    # update batcher stats
    if _p4_batcher is not None:
        _p4_batcher["total_pushed"]  += n
        _p4_batcher["total_flushed"] += n   # sync path: pushed == flushed
    ...
    return JSONResponse({
        ...
        "pushed":       _p4_batcher["total_pushed"]  if _p4_batcher else n,
        "flushed":      _p4_batcher["total_flushed"] if _p4_batcher else n,
    })

@app.get("/p4/stats")
def p4_stats():
    """S40: P4 batcher counters + config."""
    if not _HAS_P4:
    ...
    return JSONResponse({
        ...
        "total_pushed": _p4_batcher["total_pushed"]  if _p4_batcher else 0,
        "total_flushed":_p4_batcher["total_flushed"] if _p4_batcher else 0,
        "pending":      len(_p4_batcher["pending"])  if _p4_batcher else 0,
    })
...
def health():
    ...
    return {
        ...
        "contexts": len(_contexts), "version": "S40",
        ...
        "endpoints": [
            ...
            "POST /p4/route                 [S40: geo route dispatch AVX2|CUDA]",
            "GET  /p4/stats                 [S40: batcher counters]",
        ],
    }

if __name__ == "__main__":
    import uvicorn
    uvicorn.run("rest_server_s40:app", host=_HOST, port=_PORT,
