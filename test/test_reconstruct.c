...
static void _make_chunk(uint8_t *dst, uint8_t fill, uint8_t flags, uint32_t payload_len)
{
    ...
    hdr.qrpn_cg = phi_scatter_hex((uint64_t)xf ^ 0);  /* seq=0 placeholder */
...
static void fixture_init(TestFixture *tf, int n_chunks, uint8_t fill)
{
    ...
    for (int i = 0; i < n_chunks; i++) {
        ...
        se.scan_offset = (uint64_t)(i * 64);
        se.seq         = (uint32_t)i;
        se.scan_flags  = 0;
        /* orig_checksum = XOR-fold of fill×64 */
        uint32_t xf = 0;
        for (int j = 0; j < 16; j++) { uint32_t w; memset(&w, fill, 4); xf ^= w; }
        se.orig_checksum = xf;
        ...
        file_index_add(&tf->fi, &se, &ch, (uint64_t)(i * per_chunk));
    }
