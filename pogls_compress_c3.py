...
class C3WalletBridge(io.RawIOBase):
    ...
    def _probe_incompressible(raw: bytes, window_size: int,
        ...
        """
        Encode first probe_chunks (max) via C1 — if >90% are RAW frames,
        declare the data incompressible and use passthrough.
        Cost: ~1.5ms per 32 chunks (negligible vs full encode).
        """
        from pogls_compress_c1 import C1Encoder, _is_zero, _phi_position, _sparse_diff
        BS_  = BS
        ...
        for i in range(n):
            ...
            if _is_zero(chunk):
                enc._record(i, chunk); continue
            spoke, _ = _phi_position(i)
            bk       = spoke * enc._phi_buckets // 6
            pm       = enc._phi_lookup(chunk, bk)
            if pm is not None:
                base = enc._abs_idx - len(enc._history)
                ref  = enc._history[pm - base]
                if _sparse_diff(chunk, ref) is not None:
                    enc._record(i, chunk); continue
            dm = enc._delta_lookup(chunk)
            if dm is not None:
                base = enc._abs_idx - len(enc._history)
                ref  = enc._history[dm - base]
                if _sparse_diff(chunk, ref) is not None:
                    enc._record(i, chunk); continue
            raw_ += 1
            enc._record(i, chunk)
