...
static inline uint8_t te_eval_state(const GeoSnap *s) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (diff < 0) diff = -diff;
        if (diff > (int)QRPN_IMBAL_THRESH) imbal = 1;  /* > 72 */
    }
    /* check density */
...
static inline void te_tick(ThirdEye *te, GeoSeed cur, uint8_t spoke, uint8_t slot_hot,
                           uint32_t val_drift) {
    te->op_count++;
    te->cur.spoke_count[spoke % 6]++;
    if (slot_hot) te->cur.hot_slots++;
    /* val_drift: value-based cross-byte drift → weight into hot_slots
     * diff=0..7, weight = diff>>1 (0-3) → max +3/op, threshold still valid
     * unlikely to fire every op → amortized cost ≈ 0 */
    if (val_drift > 2u) {
