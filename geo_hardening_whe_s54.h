...
/* ════════════════════════════════════════════════════════════════════
 * AVX2 x4 path — geo_fast_intersect_x4 + geo_fused_write_batch_x4
 *
 * S54: process 4 raw_in values per iteration using DiamondFlowCtx4.
 *
 * Pipeline per iteration:
 *   theta_mix64 x4  (scalar unroll — no SIMD theta, amortised over loop)
 *   core_raw pack   (scalar → _mm256_loadu into 1 register)
 *   geo_fast_intersect_x4  (3× AVX2 rotr + AND = 9 instructions)
 *   diamond_batch_temporal_x4  (route accumulate + drift + flow_end)
 *   geo_route_addr_guard_x4    (branchless via cmpeq + blendv)
 *   dodeca_insert per lane that fired (checked via resets bitmask)
 *
 * WHE: not wired in x4 path (WHE requires per-step expected_r from
 *   face/edge — maintaining 4 WheCtx in parallel adds more cost than
 *   it saves at current batch sizes). Caller may run WHE on scalar
 *   fallback tail.
 *
 * Fallback tail: if n % 4 != 0, remaining cells are processed by
 *   geo_fused_write_batch (scalar) so no element is dropped.
 * ════════════════════════════════════════════════════════════════════ */

#ifdef __AVX2__
#include <immintrin.h>

/* geo_fast_intersect x4 — 4 core_raw values in one __m256i
 * identical logic to scalar: raw & ror8 & ror16 & ror24
 * ror k bits: srli(k) | slli(64-k) */
static inline __m256i geo_fast_intersect_x4(__m256i raw) {
    __m256i r8  = _mm256_or_si256(_mm256_srli_epi64(raw,  8), _mm256_slli_epi64(raw, 56));
    __m256i r16 = _mm256_or_si256(_mm256_srli_epi64(raw, 16), _mm256_slli_epi64(raw, 48));
    __m256i r24 = _mm256_or_si256(_mm256_srli_epi64(raw, 24), _mm256_slli_epi64(raw, 40));
    return _mm256_and_si256(raw, _mm256_and_si256(r8, _mm256_and_si256(r16, r24)));
}

/* geo_route_addr_guard x4 — branchless PHI inject on degenerate lanes
 * degenerate = (route_addr & TOPO_MASK) == 0
 * blendv_epi8: selects b where mask byte is 0xFF, else a */
static inline __m256i geo_route_addr_guard_x4(__m256i route, __m256i isect) {
    const __m256i topo  = _mm256_set1_epi64x((int64_t)TOPO_MASK);
    const __m256i phi   = _mm256_set1_epi64x((int64_t)GEO_SEED_GUARD);
    const __m256i zero  = _mm256_setzero_si256();
    /* mask = 0xFFFF...FF where (route & TOPO_MASK) == 0 */
    __m256i masked = _mm256_and_si256(route, topo);
    __m256i is_deg = _mm256_cmpeq_epi64(masked, zero);           /* FF on degenerate */
    /* inject = phi ^ isect — applied only to degenerate lanes */
    __m256i inject = _mm256_xor_si256(phi, isect);
    __m256i fixed  = _mm256_xor_si256(route, inject);            /* route ^ phi ^ isect */
    return _mm256_blendv_epi8(route, fixed, is_deg);             /* blend: fixed if degenerate */
}

/* geo_fused_write_batch_x4
 *
 * Processes n inputs 4 at a time (bulk) then tails via scalar.
 * ctx4   : carry-across-batch for AVX2 lanes (DiamondFlowCtx4)
 * ctx    : carry-across-batch for scalar tail  (DiamondFlowCtx)
 * All other args mirror geo_fused_write_batch.
 *
 * Returns total dna_writes (bulk + tail).
 */
static inline uint32_t geo_fused_write_batch_x4(
...
{
    uint32_t dna_writes = 0;
    uint32_t bulk = n & ~3u;    /* round down to multiple of 4 */

    /* ── bulk: 4 lanes per iteration ────────────────────────────── */
    for (uint32_t i = 0; i < bulk; i += 4) {

        /* theta_mix64 x4 — scalar unroll (no SIMD theta) */
        uint64_t h0 = theta_mix64(raw_in[i+0]);
        uint64_t h1 = theta_mix64(raw_in[i+1]);
        uint64_t h2 = theta_mix64(raw_in[i+2]);
        uint64_t h3 = theta_mix64(raw_in[i+3]);

        /* face/edge pack → core_raw x4 */
        uint64_t cr[4];
        cr[0] = (((uint64_t)(uint8_t)(((uint64_t)(uint32_t)(h0>>32) * 12u) >> 32)) << 59)
              | (((uint64_t)(uint8_t)(((uint64_t)(uint32_t)(h0     ) *  5u) >> 32)) << 52)
              | (raw_in[i+0] & UINT64_C(0x000FFFFFFFFFFFFF));
        cr[1] = (((uint64_t)(uint8_t)(((uint64_t)(uint32_t)(h1>>32) * 12u) >> 32)) << 59)
              | (((uint64_t)(uint8_t)(((uint64_t)(uint32_t)(h1     ) *  5u) >> 32)) << 52)
              | (raw_in[i+1] & UINT64_C(0x000FFFFFFFFFFFFF));
        cr[2] = (((uint64_t)(uint8_t)(((uint64_t)(uint32_t)(h2>>32) * 12u) >> 32)) << 59)
              | (((uint64_t)(uint8_t)(((uint64_t)(uint32_t)(h2     ) *  5u) >> 32)) << 52)
              | (raw_in[i+2] & UINT64_C(0x000FFFFFFFFFFFFF));
        cr[3] = (((uint64_t)(uint8_t)(((uint64_t)(uint32_t)(h3>>32) * 12u) >> 32)) << 59)
              | (((uint64_t)(uint8_t)(((uint64_t)(uint32_t)(h3     ) *  5u) >> 32)) << 52)
              | (raw_in[i+3] & UINT64_C(0x000FFFFFFFFFFFFF));

        /* geo_fast_intersect_x4 */
        __m256i cr4    = _mm256_loadu_si256((const __m256i*)cr);
        __m256i isect4 = geo_fast_intersect_x4(cr4);

        /* route accumulate via diamond_batch_temporal_x4 (1 step) */
        /* isect4 stored to aligned buffer for batch_temporal_x4 input */
        uint64_t isect_buf[4];
        _mm256_storeu_si256((__m256i*)isect_buf, isect4);
        uint32_t resets = diamond_batch_temporal_x4(ctx4, isect_buf, 1, baseline);

        /* guard degenerate lanes on route_addr */
        __m256i r4 = _mm256_loadu_si256((const __m256i*)ctx4->route_addr);
        r4 = geo_route_addr_guard_x4(r4, isect4);
        _mm256_storeu_si256((__m256i*)ctx4->route_addr, r4);

        /* DNA write for lanes that fired (resets bitmask) */
        if (unlikely(resets)) {
            for (int f = 0; f < 4; f++) {
                if (resets & (1u << f)) {
                    uint8_t off = (uint8_t)(__builtin_popcountll(
                        baseline & ~isect_buf[f]) & 0xFFu);
                    dodeca_insert(dodeca,
                                  ctx4->snap_route[f], 0, 0, off,
                                  (uint16_t)ctx4->snap_hop[f], segment);
                    dna_writes++;
                }
            }
        }
    ...
    /* ── scalar tail: remaining n%4 elements ─────────────────────── */
    if (bulk < n) {
        dna_writes += geo_fused_write_batch(
            raw_in + bulk, n - bulk,
            baseline, ctx, dodeca, segment, NULL, 0);
    }

    return dna_writes;
}

#endif /* __AVX2__ (geo_fused_write_batch_x4) */
