...
# ── S32: multi-layer NDJSON batch stream ──────────────────────────

def _ndjson_multi_stream(
    geo_ctx,
    reqs:        list[dict],    # [{name, file_idx, n_bytes, layer_id}, ...]
    spoke_mask:  int  = 0x3F,
    audit_only:  bool = False,
    val_min:     int  = 1,
    val_max:     int  = 0,
    ...
    """
    Yields one JSON line per batch (list of GeoMultiRec dicts).
    Last line is a _summary record.
    Downstream: parse each line as JSON array → iterate records.
    """
    ...
    for batch in geo_ctx.iter_multi_range(
        ...
        for rec in batch:
            ...
            total_records += 1
        # emit full batch as one JSON line (array)
...
async def lifespan(app: FastAPI):
    log.info("POGLS REST server (S32) starting — geo_backend=%s iter=%s multi=%s port=%d",
             _GEO_BACKEND, _HAS_ITER, _HAS_MULTI, _PORT)
    _get_ctx("default")
    yield
    log.info("POGLS REST server shutdown")

app = FastAPI(
    title="POGLS v4 REST API (S32)",
    version="1.32.0",
    description="streaming reconstruct | multi-layer batch | chunk window | flags filter",
    lifespan=lifespan,
)
...
def wallet_reconstruct(body: ReconstructBody):
    ...
    log.info(
        ...
        "chunk_win=%s val=%s mask=%s",
        body.handle_id, len(reqs), total_chunks_est,
        chunk_window_str, val_range_str, hex(spoke_mask)
    )
