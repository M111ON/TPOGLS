...
class WalletCreateBody(BaseModel):
    class FileItem(BaseModel):
        ...
        data_b64:     str
        drift_window: int = 0
    files: list["WalletCreateBody.FileItem"]

class WalletFetchBody(BaseModel):
    wallet_b64:  str
    file_idx:    int = 0
    byte_offset: int = 0
    src_b64:     str

class WalletReconBody(BaseModel):
    wallet_b64: str
    file_idx:   int = 0
    src_b64:    str
...
def wallet_create(body: WalletCreateBody):
    """Scan raw file bytes → produce .pogwallet blob (buffer mode)."""
    ...
    try:
        ...
        for item in body.files:
            raw = _b64.b64decode(item.data_b64)
            if not raw:
                raise HTTPException(400, f"file '{item.name}' is empty")
            ...
            off = 0
            while off < len(raw):
                chunk = raw[off: off + 64].ljust(64, b"\x00")
                wb.add_coord(fid, off,
                             chunk_seed(chunk), chunk_checksum(chunk),
                             drift_window=item.drift_window)
                off += 64

            file_stats.append({"name": item.name, "size": len(raw),
                                "chunks": (len(raw) + 63) // 64})

        blob = wb.seal()
        ms   = (time.perf_counter() - t0) * 1000
        log.info("wallet/create files=%d wallet_bytes=%d ms=%.1f",
                 len(body.files), len(blob), ms)
        return JSONResponse({
            "wallet_b64":   _b64.b64encode(blob).decode(),
            "wallet_bytes": len(blob),
            "files":        file_stats,
            "ms":           round(ms, 2),
        })
        ...
        raise HTTPException(500, f"wallet error {e.code}: {e}")
    except Exception as e:
        log.error("wallet/create: %s", e)
        raise HTTPException(500, str(e))


@app.post("/wallet/fetch")
def wallet_fetch(body: WalletFetchBody):
    """O(1) single chunk fetch + verify."""
    t0 = time.perf_counter()
    try:
        blob = _b64.b64decode(body.wallet_b64)
        src  = _b64.b64decode(body.src_b64)
        wr   = WalletReader(blob)

        coord_id = wr.lookup_by_offset(body.file_idx, body.byte_offset)
        if coord_id >= wr.coord_count:
            raise HTTPException(404, f"offset {body.byte_offset} not in wallet")

        chunk = wr.fetch_chunk(coord_id, src)
        cr    = wr.coord_info(coord_id)
        ms    = (time.perf_counter() - t0) * 1000
        return JSONResponse({
            "chunk_b64":   _b64.b64encode(chunk).decode(),
            "coord_id":    coord_id,
            "scan_offset": cr.scan_offset,
            "face":        cr.face,
            "edge":        cr.edge,
            "z":           cr.z,
            "ms":          round(ms, 3),
        ...
        status = 409 if e.code in (-3, -4) else 500
        raise HTTPException(status, f"wallet error {e.code}: {e}")
    except Exception as e:
        log.error("wallet/fetch: %s", e)
        raise HTTPException(500, str(e))


@app.post("/wallet/reconstruct")
def wallet_reconstruct(body: WalletReconBody):
    """Reconstruct full file from wallet + source bytes (buffer mode)."""
    t0 = time.perf_counter()
    try:
        blob = _b64.b64decode(body.wallet_b64)
        src  = _b64.b64decode(body.src_b64)
        ...
        meta_warned = []
        data = wr.reconstruct_file(
            body.file_idx, src,
            on_meta_drift=lambda fe: meta_warned.append(fe.file_size))
        ms   = (time.perf_counter() - t0) * 1000
        return JSONResponse({
            "data_b64":   _b64.b64encode(data).decode(),
            "orig_bytes": len(data),
            "meta_drift": bool(meta_warned),
            "ms":         round(ms, 2),
        })
    except HTTPException:
        raise
        ...
        log.error("wallet/reconstruct: %s", e)
        raise HTTPException(500, str(e))


@app.get("/wallet/stats")
def wallet_stats_endpoint(wallet_b64: str = Query(...)):
    """Parse wallet blob → return stats (no file I/O)."""
    try:
        blob = _b64.b64decode(wallet_b64)
        wr   = WalletReader(blob)
        return JSONResponse(wr.stats())
    except WalletError as e:
        raise HTTPException(400, f"wallet error {e.code}: {e}")
    except Exception as e:
        raise HTTPException(400, str(e))
