/*
 * geo_wire_kernel.cu — S28 wire integration
 * Exposes geo_wire_launch_async(): K6 fused32 behind a clean async API.
 *
 * Caller owns all CUDA memory. This file adds no extra allocations.
 *
 * Compile: nvcc -O2 -arch=sm_75 -o geo_wire geo_wire_kernel.cu geomatrix_kernel.cu
 */

#include <stdint.h>
#include <stdio.h>
#include "geo_wire_types.h"   /* GeoPacket32, GeoWireHandle, declarations */

/* ── geo_wire_launch_async ──────────────────────────────────────────
 * Fires H2D + K6 + optional D2H in a single stream.
    ...
    int                n_packets)
{
    ...
    geo_isect_fused32<<<grid, 256, 0, h->stream>>>(
 ...
 * Split N_packets across N_STREAMS — each stream: H2D → K6 → D2H.
    ...
    cudaStream_t      *streams)
{
    ...
    for (int s = 0; s < N_STREAMS; s++) {
        ...
        geo_isect_fused32<<<grid, 256, 0, streams[s]>>>(
