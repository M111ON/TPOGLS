 ...
 * Replaces smem XOR-reduce + 4×__syncthreads() with 3×__shfl_xor_sync.
...
#define MAX_STREAMS 8
     ...
     * Replaces 4×__syncthreads() with 3×__shfl_xor_sync + 1 smem broadcast.
    ...
    /* ═══════════════════════════════════════════════════════════════
     * S29-A: Triple-buffer pipeline — overlap H2D / K7 / D2H
     * 3 streams, 3 pinned host buffers, each chunk = N_PACKETS/3.
     * While stream[i] runs kernel on chunk[i]:
     *   stream[i-1] D2H  ← results from previous chunk
     *   stream[i+1] H2D  → preload next chunk
     * Targets PCIe overhead reduction (was 12.15× in K6 serial).
     * ══════════════════════════════════════════════════════════════ */
    {
#define N_PIPE 3
        int    chunk_p    = N_PACKETS / N_PIPE;
        int    chunk_bund = chunk_p / GEO_BUNDLE_WORDS;
        int    chunk_grid = (chunk_bund + 31) / 32;
        size_t pkt_sz     = sizeof(GeoPacket32);
        size_t chunk_pkt_bytes = (size_t)chunk_p * pkt_sz;
        size_t chunk_res_bytes = (size_t)chunk_p;

        /* 3 device + 3 host packet buffers + 3 result buffers */
        ...
        for (int i = 0; i < N_PIPE; i++) {
            CUDA_CHECK(cudaMalloc   (&d_pipe_pkts[i], chunk_pkt_bytes));
            CUDA_CHECK(cudaMallocHost(&h_pipe_pkts[i], chunk_pkt_bytes));
            CUDA_CHECK(cudaMalloc   (&d_pipe_res [i], chunk_res_bytes));
            CUDA_CHECK(cudaMallocHost(&h_pipe_res [i], chunk_res_bytes));
            CUDA_CHECK(cudaStreamCreate(&pipe_s[i]));
            /* fill host packets — same expected sig for correctness */
            for (int j = 0; j < chunk_p; j++) {
        ...
        /* Steady-state pipeline: each iteration processes one chunk.
         * Pipeline fill happens on first 2 iterations naturally.
         * Total: N_PIPE chunks back-to-back → end-to-end for all N_PACKETS. */
        int N_ITER = N_PIPE * 10;   /* 10 full passes for stable timing */

        /* warmup */
        for (int i = 0; i < N_PIPE; i++) {
            int slot = i % N_PIPE;
            CUDA_CHECK(cudaMemcpyAsync(d_pipe_pkts[slot], h_pipe_pkts[slot],
                                       chunk_pkt_bytes, cudaMemcpyHostToDevice, pipe_s[slot]));
            ...
            CUDA_CHECK(cudaMemcpyAsync(h_pipe_res[slot], d_pipe_res[slot],
                                       chunk_res_bytes, cudaMemcpyDeviceToHost, pipe_s[slot]));
        }
        ...
        /* Timed run: issue H2D+kernel+D2H for all N_ITER chunks concurrently */
        ...
        float ms_pipe; CUDA_CHECK(cudaEventElapsedTime(&ms_pipe, tp0, tp1));
        ms_pipe /= (float)N_ITER;   /* per-chunk amortized */

        /* correctness: check slot 0 results */
        int pass_pipe = 0;
        for (int j = 0; j < chunk_p; j++) pass_pipe += h_pipe_res[0][j];

        double mpkt_pipe_chunk = chunk_p / (ms_pipe * 1e-3) / 1e6;
        /* effective total throughput: N_PACKETS processed in ~ms_pipe (overlapped) */
        double mpkt_pipe_total = N_PACKETS / (ms_pipe * 1e-3) / 1e6;

        printf("\n[S29-A: Triple-buffer pipeline (H2D → K7 → D2H overlap)]\n");
        printf("  Chunk size:     %d pkts  (%d bundles)\n", chunk_p, chunk_bund);
        printf("  Amortized time: %.3f ms/chunk\n", ms_pipe);
        printf("  Throughput:     %.1f M-pkt/s (chunk)  |  %.1f M-pkt/s (effective N_PACKETS)\n",
               mpkt_pipe_chunk, mpkt_pipe_total);
        printf("  Pass rate:      %d/%d  (%.2f%%)\n",
               pass_pipe, chunk_p, 100.0 * pass_pipe / chunk_p);
        printf("  vs K6 e2e:      %.2fx  (K6 e2e = 1255.2 M-pkt/s)\n",
               mpkt_pipe_total / 1255.2);
        printf("  vs K6 kernel:   %.2fx  (K6 kernel-only = 15244.7 M-pkt/s)\n",
               mpkt_pipe_total / 15244.7);
