...
/* delegates to canonical pogls_checksum64 (defined in pogls_compress.h) */
static inline uint32_t _recon_chunk_checksum(const uint8_t *data)
{
    return pogls_checksum64(data);
}
                                           ...
                                           uint32_t           *bytes_out)
{
    ...
    uint32_t chk = _recon_chunk_checksum((const uint8_t *)&blk);
    if (chk != rec->orig_checksum) return RECON_ERR_CHECKSUM;

    /* ── 4. Record integrity: recompute _fidx_rec_crc and compare ── */
    /*
     * rec_crc = XOR-fold of all record fields (from _fidx_rec_crc).
     * We verify by recomputing over the stored record.
     * This catches any in-memory corruption of the FileChunkRecord.
     */
    {
        /* XOR-fold first 40B of record (all fields before rec_crc at offset 40) */
        uint32_t crc_acc = 0, w;
        for (int _i = 0; _i < 10; _i++) {
            memcpy(&w, (const uint8_t *)rec + _i * 4, 4);
            crc_acc ^= w;
        }
        if (crc_acc != rec->rec_crc) return RECON_ERR_QRPN;
    }
...
{
    if (!rc || !out_buf || !bytes_out) return RECON_ERR_NULL;
    /* ── pre-check: fail fast before loop, not mid-stream ── */
    if (out_cap < rc->fi->hdr->orig_size) return RECON_ERR_OVERFLOW;
